...
class PhaseModel(Base):
    ...
    default_view: Mapped[DefaultView] = mapped_column(Enum(DefaultView, name='defaultview', create_type=False),
                                                      nullable=False)
    status: Mapped[PhaseStatus] = mapped_column(Enum(PhaseStatus, name='phasestatus', create_type=False),
                                                nullable=False, default=PhaseStatus.ACTIVE, index=True)
