        ...
        for page_data in pages_to_create:
            try:
                create_page_command = CreateCompanyPageCommand(
                    company_id=company_id.value,
                    page_type=MixedHelper.get_string(page_data["page_type"]),
                    title=MixedHelper.get_string(page_data["title"]),
                    html_content=MixedHelper.get_string(page_data["html_content"]),
                    meta_description=MixedHelper.get_optional_string(page_data["meta_description"]),
                    meta_keywords=MixedHelper.get_string_list(page_data["meta_keywords"]),
                    language=MixedHelper.get_string(page_data["language"]),
                    is_default=MixedHelper.get_boolean(page_data["is_default"])
                )
