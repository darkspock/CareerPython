...
class InitializeSampleDataCommandHandler(CommandHandler[InitializeSampleDataCommand]):
    ...
    def _create_sample_candidates(self, num_candidates: int) -> list[CandidateId]:
        ...
        cities = [
            "San Francisco", "New York", "Austin", "Seattle", "Boston", "Los Angeles", "Chicago", "Denver", "Miami",
            "Portland"
        ]
            ...
            for phase_id in phases:
                ...
                if workflow_result:
                    workflows_info[workflow_result[0]] = {
                        'initial_stage_id': WorkflowStageId.from_string(workflow_result[1]) if workflow_result[
                            1] else None
                    }
