...
from src.company_workflow.application.queries.get_all_custom_field_values_by_company_candidate import \
    GetAllCustomFieldValuesByCompanyCandidateQuery
...
class CustomFieldValueController:
    ...
    def update_custom_field_value(self, value_id: str, request: UpdateCustomFieldValueRequest) -> Optional[
        ...
        if not values_to_update and request.field_value is not None:
            ...
            raise ValueError(
                "Cannot update with field_value alone. Use values dict or upsert_single_field_value endpoint.")
    ...
    def upsert_single_field_value(
        ...
        workflow_id = CompanyWorkflowId(custom_field_dto.workflow_id)
        ...
        if existing_value:
            ...
            if not result:
                raise ValueError("Failed to update custom field value")
            return result
        else:
