...
class CustomFieldValueModel(Base):
    ...
    company_candidate_id: Mapped[str] = mapped_column(String(255), ForeignKey("company_candidates.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    values: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)  # JSON with all field values, keyed by field_id (not field_key, to avoid data loss on rename)
