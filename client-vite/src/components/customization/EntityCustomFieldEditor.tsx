...
import { FieldConfigEditor } from '../workflow/FieldConfigEditor';
...
export const EntityCustomFieldEditor: React.FC<EntityCustomFieldEditorProps> = ({
    ...
    try {
      ...
      if (err?.status === 404 || err?.message?.includes('404')) {
        try {
          ...
          setCustomizationId(newCustomization.id);
          setFields([]);
          if (notifyParent) {
            onFieldsChange?.([]);
          }
  ...
  const handleFieldNameChange = (value: string) => {
    const fieldKey = EntityCustomizationService.generateFieldKey(value);
    setFormData(prev => ({
      ...prev,
      field_name: value,
      field_key: fieldKey
  ...
  const saveFields = async (updatedFields: CustomField[]) => {
    if (!customizationId) {
      throw new Error('Customization ID not found');
    }

    // Convert CustomField[] to CreateCustomFieldRequest[]
    const fieldsRequest: CreateCustomFieldRequest[] = updatedFields.map(field => ({
      field_key: field.field_key,
      field_name: field.field_name,
      field_type: field.field_type,
      field_config: field.field_config || undefined,
      order_index: field.order_index
    }));

    await EntityCustomizationService.updateCustomization(customizationId, {
    ...
    try {
      ...
      if (isAddingNew) {
        ...
        const newField: CustomField = {
          id: `temp_${Date.now()}`, // Temporary ID, will be replaced after save
          field_key: formData.field_key,
          field_name: formData.field_name,
          field_type: formData.field_type,
          field_config: Object.keys(formData.field_config).length > 0 ? formData.field_config : null,
  ...
  const handleMoveUp = async (field: CustomField) => {
    ...
    try {
      const updatedFields = fields.map(f => {
        if (f.id === field.id) {
          return { ...f, order_index: f.order_index - 1 };
        } else if (f.order_index === field.order_index - 1) {
          return { ...f, order_index: f.order_index + 1 };
        }
        return f;
