...
const CompanyInterviewsPage: React.FC = () => {
    ...
    // Maps only for filter dropdowns (positions, roles, users) - not for displaying interview data
    ...
    const [dateFilterBy, setDateFilterBy] = useState<'scheduled' | 'deadline' | 'unscheduled'>('scheduled'); // Legacy, kept for date range filtering
    ...
    // Load filter options (positions, roles, users) once on mount
    useEffect(() => {
        let ignore = false;

        async function loadFilterOptions() {
            const companyId = getCompanyId();
            if (!companyId || ignore) return;
    ...
    // Load interviews and stats when filters change
    useEffect(() => {
        let ignore = false;
    ...
    const loadInterviews = async (overrideFilters?: {
        ...
        fromDate?: string;
        toDate?: string;
        filterBy?: 'PENDING_TO_PLAN' | 'PLANNED' | 'IN_PROGRESS' | 'RECENTLY_FINISHED' | 'OVERDUE' | 'PENDING_FEEDBACK';
    }) => {
        try {
            setLoading(true);
            setError(null);
            ...
            // Handle filter_by: if overrideFilters has filterBy, use it (it's an InterviewFilterEnum value)
            // Otherwise, if we have date filters, use dateFilterBy for date field selection
            ...
            if (overrideFilters?.filterBy) {
                // Use InterviewFilterEnum value from overrideFilters
                filters.filter_by = overrideFilters.filterBy as any;
            } else if (effectiveFromDate || effectiveToDate) {
                // For date range filtering, use dateFilterBy to specify which date field
                filters.filter_by = dateFilterBy as any;
            ...
            // All information (candidate names, position titles, interviewer names, role names)
            // is now included directly in the response from the backend ReadModel
    ...
    const handleFilterByMetric = (filterType: string) => {
        ...
        // Now using InterviewFilterEnum values
        let filterOverrides: {
            status?: string;
            fromDate?: string;
            toDate?: string;
        ...
        switch (filterType) {
            case 'pending_to_plan':
                // Use filter_by=PENDING_TO_PLAN (backend handles the logic)
                filterOverrides.filterBy = 'PENDING_TO_PLAN';
                break;
            case 'planned':
                // Use filter_by=PLANNED (backend handles the logic)
                filterOverrides.filterBy = 'PLANNED';
                break;
            case 'in_progress':
                // Use filter_by=IN_PROGRESS (backend handles the logic)
                filterOverrides.filterBy = 'IN_PROGRESS';
                break;
            case 'recently_finished':
                // Use filter_by=RECENTLY_FINISHED (backend handles the logic)
                filterOverrides.filterBy = 'RECENTLY_FINISHED';
                break;
            case 'overdue':
                // Use filter_by=OVERDUE (backend handles the logic)
                filterOverrides.filterBy = 'OVERDUE';
                break;
            case 'pending_feedback':
                // Use filter_by=PENDING_FEEDBACK (backend handles the logic)
                            ...
                            <div className="lg:col-span-2">
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    ...
                                    >
                                        ...
                                        <CardContent>
                                            <div
                                                className="text-2xl font-bold text-orange-600">{stats.pending_to_plan || 0}</div>
                                        </CardContent>
                                    </Card>
                                    <Card
                                        className="cursor-pointer hover:shadow-md transition-shadow"
                                    ...
                                    >
                                        ...
                                        <CardContent>
                                            <div
                                                className="text-2xl font-bold text-green-600">{stats.in_progress_interviews || 0}</div>
                                        </CardContent>
                                    </Card>
                                    <Card
                                        className="cursor-pointer hover:shadow-md transition-shadow"
                                    ...
                                    >
                                        ...
                                        <CardContent>
                                            <div
                                                className="text-2xl font-bold text-purple-600">{stats.recently_finished || 0}</div>
                                        </CardContent>
                                    </Card>
                                    <Card
                                        className="cursor-pointer hover:shadow-md transition-shadow"
                                    ...
                                    >
                                        ...
                                        <CardContent>
                                            <div
                                                className="text-2xl font-bold text-yellow-600">{stats.pending_feedback || 0}</div>
                                        </CardContent>
                                    </Card>
                                </div>
                            </div>
                                                ...
                                                {Array.from({length: 35}, (_, i) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {interviewCount > 0 && (
                                                                <div
                                                                    className="text-[8px] text-blue-600 font-bold mt-0.5">
                                                                    {interviewCount}
                                                                </div>
                                                            )}
                                                        </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col md:flex-row gap-4">
                                    ...
                                    <Select value={typeFilter} onValueChange={setTypeFilter}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="KNOWLEDGE_CHECK">Verificación de
                                                Conocimientos</SelectItem>
                                            <SelectItem value="EXPERIENCE_CHECK">Verificación de
                                                Experiencia</SelectItem>
                                        </SelectContent>
                                    </Select>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    >
                                        ...
                                        Filtros Avanzados
                                        {showAdvancedFilters ? <ChevronUp className="w-4 h-4"/> :
                                ...
                                {showAdvancedFilters && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t">
                                        <div>
                                            ...
                                            <Select value={processTypeFilter} onValueChange={setProcessTypeFilter}>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="CANDIDATE_SIGN_UP">Registro de
                                        ...
                                        <div>
                                            <Label className="text-xs text-gray-600 mb-1 block">Filtrar por
                                                Fecha</Label>
                                            <Select value={dateFilterBy}
                                                ...
                                                <TableCell>
                                                    <div className="flex flex-col">
                                                        {interview.title && interview.title.trim() ? (
                                                            <span
                                                                className="font-medium text-gray-900">{interview.title}</span>
                                                    ...
                                                    {interview.interviewers && interview.interviewers.length > 0 ? (
                                                        <div className="flex flex-col gap-1">
                                                            ...
                                                            {((interview.interviewer_names || interview.interviewers || []).length > 2) && (
                                                                <span
                                                                    className="text-xs text-gray-400">+{(interview.interviewer_names || interview.interviewers || []).length - 2} más</span>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            ...
                                                            {(interview.required_role_names || interview.required_roles || []).length > 2 && (
                                                                <span
                                                                    className="text-xs text-gray-400">+{(interview.required_role_names || interview.required_roles || []).length - 2} más</span>
                                                            )}
                                                        </div>
                                                    ) : (
